...
class PhieuDeXuatMuaHangItem extends StatelessWidget {
  const PhieuDeXuatMuaHangItem(
      {Key? key, required this.phieuDXMH, this.showAction = true})
      : super(key: key);
  final PhieuDeXuatMuaHang phieuDXMH;
  final bool showAction;

  @override
  Widget build(BuildContext context) {
    return _builPhieuDeXuatMuaHangItem(context: context, item: phieuDXMH);
  }

  Widget _builPhieuDeXuatMuaHangItem(
      {required BuildContext context, required PhieuDeXuatMuaHang item}) {
    final dateFmt = DateFormat('dd/MM/yyyy');
    final moneyFmt = NumberFormat("#,###", "en_US");

    final ngayDeXuatText = item.ngayDeXuat == null
        ? "Ngày …. tháng … năm …"
        : dateFmt.format(item.ngayDeXuat!);
    final tienDeXuatText = moneyFmt.format(item.tongTien);

    return GestureDetector(
      onTap: () {
        MyNavigation.intentWithData(
          ScreenRouteName.previewDXMHPage,
          arguments: DocumentArgs(
              maCongTy: item.maCongTy,
              reportId: item.idDeXuatMh,
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              StyledText(
                                text:
                                    'Tiền đề nghị: <bold>$tienDeXuatText</bold>',
                                tags: {
                                  'bold': StyledTextTag(
                                      style: const TextStyle(
                                          fontWeight: FontWeight.bold)),
