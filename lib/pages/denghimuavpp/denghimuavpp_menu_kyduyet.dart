...
class MenuDeNghiMuaVpp extends StatefulWidget {
  final DeNghiMuaVppArgs args;
  final Future<bool> Function(bool isApproved, DeNghiMuaVppArgs data)
      onApproved;

  final BuildContext? parentContext;
  const MenuDeNghiMuaVpp(
      {Key? key,
      required this.args,
  ...
  @override
  State<MenuDeNghiMuaVpp> createState() => _MenuDeNghiMuaVppState();
}

class _MenuDeNghiMuaVppState extends State<MenuDeNghiMuaVpp> {
  ...
  Widget build(BuildContext context) {
    if ([4].contains(widget.args.tinhTrang)) {
  ...
  Future<void> doApprove(bool approved, String noiDung) async {
    ...
    try {
      if (isOk) {
        ...
        mainModel.giayDNMuaVppStream.add(widget.args.idDeNghi!);
