...
class MenuYeuCauThietKe extends StatefulWidget {
  final YeuCauThietKeArgs args;
  final Future<bool> Function(bool isApproved, YeuCauThietKeArgs data)
      onApproved;

  final BuildContext? parentContext;
  const MenuYeuCauThietKe(
      {Key? key,
      required this.args,
  ...
  @override
  State<MenuYeuCauThietKe> createState() => _MenuYeuCauThietKeState();
}

class _MenuYeuCauThietKeState extends State<MenuYeuCauThietKe> {
  ...
  Future<void> doApprove(bool approved, String noiDung) async {
    BuildContext parentContext = widget.parentContext!;
    ...
    if (isOk) {
      UIHelper.showOk(parentContext, "Duyệt thành công hồ sơ.");
      final mainModel = parentContext.read<MainPageModel>();
      mainModel.documentSignedStream.add(SignedStreamArgs(
          DataHelper.phieuYeuCauThietKeName, (widget.args.idAutoQC!)));
      mainModel.documentChartStream.add(widget.args.idAutoQC!);
      // ignore: dead_code
    } else {
      UIHelper.showError(parentContext, "Không thể duyệt hồ sơ.");
    }
  }
}
